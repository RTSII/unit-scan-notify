...
const Index = () => {
  ...
  return <div className="min-h-screen bg-gradient-to-br from-vice-purple via-black to-vice-blue">
      {/* Header */}
      <div className="flex items-center justify-between p-4 bg-black/20 backdrop-blur-sm border-b border-vice-cyan/20">
        <div className="flex items-center gap-4">
          <Button onClick={() => navigate('/dashboard')} variant="outline" size="sm" className="bg-black/30 border-vice-cyan/50 text-white hover:bg-vice-cyan/20">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button onClick={signOut} variant="outline" size="sm" className="bg-black/30 border-vice-cyan/50 text-white hover:bg-vice-cyan/20">
      ...
      <div className="flex justify-center p-4">
        <div className="flex bg-black/30 rounded-lg p-1 backdrop-blur-sm border border-vice-cyan/20">
          {tabs.map(tab => {
          const Icon = tab.icon;
          return <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-2 px-4 py-2 rounded-md transition-all ${activeTab === tab.id ? 'bg-vice-pink text-white' : 'text-vice-cyan hover:text-white hover:bg-white/10'}`}>
